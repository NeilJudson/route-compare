            ...
            while s:
                ...
                if ip:
                    if re.search('is[a-z ]*subnetted', s):
                        net_mask_0 = re.search(r'/[1-3]?\d', ip.group())
                        if net_mask_0:
                            net_mask = net_mask_0.group()
                    else:
                        sub_net_mask = re.search(r'/[1-3]?\d', ip.group())
                        if sub_net_mask:
                            net_ip = ip.group()
                        else:
                            net_ip = ip.group() + net_mask
                        route_detail = self.get_route_detail(s)  # 解析出route_detail
                        dic_route_table.setdefault(net_ip, route_detail)
                s = f.readline()
...
if __name__ == '__main__':
    ...
    print()
    print([(k,  dic_route_table_1[k]) for k in sorted(dic_route_table_1.keys())])  # 排序
